...
def get_lead(v_ego: float, ready: bool, tracks: dict[int, Track], lead_msg: capnp._DynamicStructReader,
  ...
  if low_speed_override:
    ...
    if v_ego > CRUISING_SPEED and not lead_dict['status'] and frogpilot_toggles.far_lead_tracking:
      far_lead_tracks = [c for c in tracks.values() if abs(c.yRel + interp(c.dRel, model_data.position.x, model_data.position.y)) < LANE_WIDTH / 2 and (c.dRel > 100 or c.vLead > 1)]
